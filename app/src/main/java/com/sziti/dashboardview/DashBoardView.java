...
//		canvas.drawLine(mCircleRightX, mCircleRightY + mRadius + progressWidth / 2f, mViewCenterX + mRadius / 2f + dp2px(10), mCircleRightY + mRadius + progressWidth / 2f, mPaint);
		...
		if (texts_index < 100)
			canvas.drawText(String.valueOf(texts_index), mViewCenterX - progressWidth, mViewCenterY + mRadius + progressWidth * 2, mPaint);
		else
			canvas.drawText(String.valueOf(texts_index), mViewCenterX - progressWidth * 1.8f, mViewCenterY + mRadius + progressWidth * 2, mPaint);
		//画文字刻度
			...
			if (i < 1) {
				if (t1 > 0) {//画完0-10 并且画下一阶段
					if (t1 == 1 && t2 == 0){//刚好为10的情况下
						...
						break;
					}else if (t1 >= 1 && t2 >= 0) {//超过10的情况下
						c.drawLine(mViewCenterX - mRadius / 2f - dp2px(10), mCircleLeftY + mRadius + progressWidth / 2f, mCircleLeftX, mCircleLeftY + mRadius + progressWidth / 2f, mPaint);
						i += 1;
						continue;
					}
				} else {//画完0-10 直接结束  小于0的情况下
					if (t2 > 0)
